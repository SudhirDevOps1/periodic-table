...
const transformLayout = {
    ...
    cone: () => {
        ...
        cards.forEach(($el, i) => {
            ...
            // Radius decreases as we go up (increasing i, decreasing y in DOM order, but here y is inverted?)
            // element 0 is at top ? No, simple loop.
            // Let's make it simple: Base at bottom.
            const p = i / cards.length;
            const radius = baseRadius * (1 - p);
    ...
    torus: () => {
        pointer.rotateX = 10;
        pointer.rotateY = 0;
        const majorRadius = 400;
        const minorRadius = 150;
        const cols = 20; // Elements per ring
        ...
        cards.forEach(($el, i) => {
            ...
            const theta = (col / cols) * Math.PI * 2; // Angle around the tube
            const phi = (row / (cards.length / cols)) * Math.PI * 2; // Angle of the tube
            ...
            // Orient element to face outward from the torus surface
            // Normal vector is (cos(theta)cos(phi), cos(theta)sin(phi), sin(theta))
            const pitch = Math.atan2(z, Math.hypot(x, y)); // This is rough, let's rely on standard lookat-ish behavior or just position.

            // Simplified rotation to face somewhat correctly
            $el.style.transform = `translate3d(${x}px, ${y}px, ${z}px) rotateY(${yaw}rad) rotateX(${theta}rad) translateZ(${offsetZ}px)`;
        });
    },
    grid: () => {
        pointer.rotateX = 10;
